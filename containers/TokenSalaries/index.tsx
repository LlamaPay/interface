...
import { IncomingTokenSalaries } from './Incoming';
import { OutgoingTokenSalaries } from './Outgoing';

export const TokenSalaries = ({
  userAddress,
  chainId,
  isIncoming,
}: {
  ...
  return (
    <Layout className="flex flex-col gap-9">
      <>
        {isIncoming ? (
          <IncomingTokenSalaries userAddress={finalAddress} chainId={finalChainId} />
        ) : (
          <OutgoingTokenSalaries userAddress={finalAddress} chainId={finalChainId} />
        )}
      </>
    </Layout>
  );
